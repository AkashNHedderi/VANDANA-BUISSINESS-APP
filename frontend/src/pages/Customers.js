...
function CustomerForm({ open, onClose, onSaved }) {
  const [f, setF] = useState({ name: "", mobile: "", address: "", gstin: "", credit_limit: 0, credit_days: 30 });
  const save = async () => {
    if (!f.name.trim()) return toast.error("Name required");
    await api.post("/customers", f);
    markSaved(); toast.success("Customer added"); onSaved(); onClose();
    setF({ name: "", mobile: "", address: "", gstin: "", credit_limit: 0, credit_days: 30 });
  };
  return (
  ...
  return (
    <div className="rise">
      <PageHeader title="CUSTOMERS">
        ...
        <Button onClick={() => setOpen(true)} data-testid="new-customer-btn"><Plus size={16} className="mr-1" /> CUSTOMER</Button>
      </PageHeader>
      <div className="border border-border rounded-sm bg-card overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-secondary/50 text-muted-foreground font-mono text-xs">
            <tr><th className="text-left p-3">Name</th><th className="text-left p-3">Mobile</th><th className="text-left p-3">GSTIN</th><th className="text-right p-3">Outstanding</th></tr>
          </thead>
          <tbody className="divide-y divide-border" data-testid="customers-table">
            {list.length === 0 && <tr><td colSpan={4} className="p-6 text-center text-muted-foreground">No customers yet.</td></tr>}
            {list.map((c) => (
