...
function PaymentForm({ open, onClose, onSaved, customers, suppliers }) {
  const [f, setF] = useState({ type: "customer", party_id: "", party_name: "", amount: "", mode: "Cash", reference: "", date: fmtDate(new Date().toISOString()) });
  ...
  const save = async () => {
    ...
    await api.post("/payments", { ...f, amount: Number(f.amount) });
    markSaved(); toast.success("Payment recorded · balances updated"); onSaved(); onClose();
    setF({ type: "customer", party_id: "", party_name: "", amount: "", mode: "Cash", reference: "", date: fmtDate(new Date().toISOString()) });
  };
  return (
...
export default function Payments() {
  ...
  useEffect(() => { load(); }, []);
  useEffect(() => { if (params.get("new") === "1") { setOpen(true); setParams({}); } }, [params]);

  return (
    <div className="rise">
      <PageHeader title="PAYMENTS">
        <Button onClick={() => setOpen(true)} data-testid="new-payment-btn"><Plus size={16} className="mr-1" /> PAYMENT</Button>
      </PageHeader>
      <div className="border border-border rounded-sm bg-card overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-secondary/50 text-muted-foreground font-mono text-xs">
            <tr><th className="text-left p-3">Date</th><th className="text-left p-3">Type</th><th className="text-left p-3">Party</th><th className="text-left p-3">Mode</th><th className="text-right p-3">Amount</th></tr>
          </thead>
          <tbody className="divide-y divide-border" data-testid="payments-table">
            {list.length === 0 && <tr><td colSpan={5} className="p-6 text-center text-muted-foreground">No payments yet.</td></tr>}
            {list.map((p) => (
      ...
      <PaymentForm open={open} onClose={() => setOpen(false)} onSaved={load} customers={customers} suppliers={suppliers} />
    </div>
  );
}
