...
async def sale_invoice(sid: str, user=Depends(get_current_user)):
    ...
    c.drawString(18 * mm, h - 12 * mm, settings.get("business_name", "Steel & Roofing Co."))
    ...
    c.drawString(18 * mm, 15 * mm, "Generated by SteelBiz")
    c.showPage()
    c.save()
    buf.seek(0)
...
async def get_settings(user=Depends(get_current_user)):
    return clean(await db.settings.find_one({"id": "app"})) or {"id": "app", "business_name": "Steel & Roofing Co.",
                                                                "gstin": "", "address": "", "phone": ""}
