...
async def find_product(name: str, specification: str = "", thickness: str = ""):
    q = {"name": {"$regex": f"^{re.escape(name.strip())}$", "$options": "i"}}
    return await db.products.find_one(q, {"_id": 0})
...
async def create_product(p: Product, user=Depends(get_current_user)):
    dup = await db.products.find_one({"name": {"$regex": f"^{re.escape(p.name.strip())}$", "$options": "i"}})
    if dup:
...
async def create_sale(s: Sale, user=Depends(get_current_user)):
    ...
    if not d.get("invoice_number"):
        n = 1001 + await db.sales.count_documents({})
        while await db.sales.find_one({"invoice_number": f"INV-{n}"}):
            n += 1
        d["invoice_number"] = f"INV-{n}"
...
async def sale_invoice(sid: str, user=Depends(get_current_user)):
    ...
    if s.get("notes"):
        ...
        c.setFillColor(colors.black)
        c.drawString(18 * mm, y, f"Note: {s.get('notes')}"[:110])
